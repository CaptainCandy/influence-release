# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 21:41:57 2019
...
data_sets = load_vehicles_special()    
...
with full_graph.as_default():
    ...
    for data_set, label in [
        ...
        inception_features_path = 'output21/%s_inception_features_new_%s.npz' % (dataset_name, label)
...
with top_graph.as_default():
    ...
    orig_weight_path = 'output21/inception_weights_%s.npy' % top_model_name
...
with top_graph.as_default():
    top_model.get_influence_on_test_loss(
        ...
        force_refresh=True)
copyfile(
    'output21/%s-cg-normal_loss-test-%s.npz' % (top_model_name, test_description),
    'output21/%s-cg-normal_loss-test-%s.npz' % (full_model_name, test_description))
