...
with full_graph.as_default():
    ...
    for data_set, label in [
        ...
        inception_features_path = 'output13/%s_inception_features_new_%s.npz' % (dataset_name, label)
...
train_f = np.load('output13/%s_inception_features_new_train.npz' % dataset_name)
train = DataSet(train_f['inception_features_val'], train_f['labels'])
test_f = np.load('output13/%s_inception_features_new_test.npz' % dataset_name)
...
with top_graph.as_default():
    ...
    orig_weight_path = 'output13/inception_weights_%s.npy' % top_model_name
...
for test_idx in range(89, num_test):
    ...
    with top_graph.as_default():
        top_model.get_influence_on_test_loss(
            ...
            force_refresh=True)
    copyfile(
        'output13/%s-cg-normal_loss-test-%s.npz' % (top_model_name, test_description),
        'output13/%s-cg-normal_loss-test-%s.npz' % (full_model_name, test_description))
